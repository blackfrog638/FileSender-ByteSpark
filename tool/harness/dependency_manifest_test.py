...
function(pkg_check_modules prefix)
  ...
  elseif(XNN_FIXTURE_UNKNOWN_SYSTEM_LIBRARY)
    set(
      Libsecret_STATIC_LIBRARIES
      "secret-1;atomic;resolv;pthread"
      PARENT_SCOPE
    )
  else()
    set(Libsecret_STATIC_LIBRARIES "secret-1;atomic;pthread" PARENT_SCOPE)
  endif()
...
function(target_link_libraries target)
  ...
  if("${target}" STREQUAL "xnn_libsecret")
    ...
    list(FIND _arguments "atomic" _atomic_index)
    if(NOT _archive_index EQUAL -1)
      set_property(GLOBAL PROPERTY XNN_ARCHIVE_LINKED TRUE)
    endif()
    if(NOT _atomic_index EQUAL -1)
      set_property(GLOBAL PROPERTY XNN_ATOMIC_LINKED TRUE)
    endif()
...
if(NOT XNN_FIXTURE_ESCAPE_PATH AND
   NOT XNN_FIXTURE_DYNAMIC_ARCHIVE AND
   NOT XNN_FIXTURE_UNKNOWN_SYSTEM_LIBRARY)
  foreach(
    _required_property
    IN ITEMS
      XNN_PROBE_ADDED
...
def validate_libsecret_resolution(root: Path) -> None:
    ...
    with tempfile.TemporaryDirectory(
        ...
        modes = (
            ...
            (
                "unknown-system-library",
                ("-DXNN_FIXTURE_UNKNOWN_SYSTEM_LIBRARY=ON",),
                False,
            ),
        )
